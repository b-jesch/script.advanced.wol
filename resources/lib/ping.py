...
def do_one(dest_addr, timeout):
    ...
    try:
        ...
        if msg.errno == 1:
            ...
            _msg = '{} - Note that ICMP messages can only be sent from processes running as root'.format(msg.strerror)
            raise socket.error(_msg)
...
def verbose_ping(dest_addr, timeout=2, count=4):
    ...
    for i in range(count):
        ...
        if delay is None:
            print("failed. (timeout within %s secs)" % timeout)
        else:
            delay = delay * 1000
